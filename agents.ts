...
export async function runAiSupportAgent ({
    ...
    const systemPrompt = await getSytemPrompt(chunks);
...
async function getSytemPrompt(chunks: MemoryRetrieveResponse[]) {
        let chunksText = '';
...
export async function runMemoryAgent(query: string) {
    const chunks: MemoryRetrieveResponse[] = await langbase.memories.retrieve({
        query,
        topK: 5,
        memory:[
            {
